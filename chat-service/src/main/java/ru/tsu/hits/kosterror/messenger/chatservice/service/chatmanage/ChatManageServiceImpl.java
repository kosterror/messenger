...
public class ChatManageServiceImpl implements ChatManageService {
    ...
    public ChatDto createGroupChat(UUID adminId, CreateUpdateChatDto dto) {
        ...
        List<RelationPerson> members = relationPersonService.createAllRelationPerson(memberIds);

        Chat chat = Chat.builder().type(ChatType.GROUP).name(dto.getChatName()).adminId(adminId).creationDate(LocalDateTime.now()).avatarId(dto.getAvatarId()).members(members).build();
    ...
    private void validateMemberIds(UUID personId, List<UUID> memberIds) {
        ...
        for (UUID memberId : memberIds) {
            try {
                ...
                throw new InternalException("Ошибка во время интеграционного запроса на " + "проверку существования связи дружбы", e);
            }
        }
